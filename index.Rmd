...
df <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", sheet = "Statistik Harian") %>% rename(date = 1, case_daily = 2, case_cum = 5, recovered_daily = 7, recovered_cum = 8, death_cum = 10, death_daily = 9, test_daily = `Orang yang dites`)
df$date <- as.Date(df$date, format = "%d %b")
#------------------ Data lokal pulau Jawa--------
provCaseCum <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", range = "Timeline!A1:AI102") %>% rename(date = 1)
provCaseDaily <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", range = "Timeline!A104:AI208", col_names = TRUE) %>% rename(date = 1)
provCaseDaily[is.na(provCaseDaily)] <- 0
provCaseDat <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", sheet = "Kasus per Provinsi", range = "Kasus per Provinsi!B2:H38")
...
idn <- read_sf("C:/Users/aldilas/Documents/Analisis Data Transaksi Halodoc/idn_map/Indonesia_Polygon.shp")
provCaseDat <- provCaseDat %>% pivot_longer(cols = -`Provinsi Asal`, names_to = "type")
provCaseMap <- merge(idn, provCaseDat, all = TRUE)
#Google Mobility data
mobility <- read_csv("C:/Users/aldilas/Downloads/Global_Mobility_Report.csv") %>% filter(sub_region_1 == "Special Region of Yogyakarta")
```
...
```{r}
valueBox(
  value = paste(format(sum(df$`Sampel baru harian`, na.rm = TRUE), big.mark = ","), "", sep = " "),
...
plot_ly(data = mobility, x = ~date) %>% 
  ...
  layout(
    ...
    xaxis = list(title = "Tanggal", rangeslider = list(type = "date")),
