...
#------------------ Data Indonesia---------------
#my_sheets <- gs_ls()
#kawalcovid <- gs_key("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw")
#df <- gs_read(kawalcovid, ws = 3)
df <- read_excel("COVID-19 di Indonesia @kawalcovid19.xlsx", sheet = "Statistik Harian", .name_repair = "minimal")
names(df)[1] <- "X1"
df <- df %>% rename(death_cum = `Meninggal
(kumulatif)`)
df <- df %>% rename(death_daily = `Meninggal
(baru)`)
...
valueBox(
  value = paste(format(sum(df$`Jumlah diperiksa`, na.rm = TRUE), big.mark = ","), "", sep = " "),
