...
provCaseCum <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", sheet = "Timeline", range = cell_rows(1:49)) %>% rename(date = 1)
provCaseDaily <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", sheet = "Timeline", range = cell_rows(51:102), col_names = TRUE) %>% rename(date = 1)
provCaseDaily[is.na(provCaseDaily)] <- 0
provCaseDat <- read_sheet("1ma1T9hWbec1pXlwZ89WakRk-OfVUQZsOCFl4FwZxzVw", sheet = "Kasus per Provinsi", range = cell_rows(2:36), col_types = "-ciii---------------------------------") %>% rename(locname = `Provinsi Asal`)
...
provCaseDat <- provCaseDat %>% pivot_longer(cols = -locname, names_to = "type")
...
plot_ly(data = df) %>%
  ...
  layout(
    ...
    xaxis = list(title = "Tanggal", range = c("2020-03-02", "2020-05-01")),
